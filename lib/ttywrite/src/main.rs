...
fn main() {
    ...
    let mut tty_settings = port.read_settings().expect("could not read tty settings");
    tty_settings.set_baud_rate(opt.baud_rate).expect("could not set baud rate");
    ...
    port.set_timeout(Duration::from_secs(opt.timeout)).expect("could not set duration");
    let input = opt.input;
    if let Some(input) = input {
        let mut reader = BufReader::new(File::open(input).expect("could not open file"));
        if opt.raw {
            io::copy(&mut reader, &mut port).expect("error copying file to port");
        } else {
            Xmodem::transmit_with_progress(reader, port, progress_fn).expect("XMODEM error");
        }
    } else {
        let mut reader = BufReader::new(io::stdin());
        if opt.raw {
            io::copy(&mut reader, &mut port).expect("error copying from stdin to port");
        } else {
            Xmodem::transmit_with_progress(reader, port, progress_fn).expect("XMODEM error");
        }
    }
