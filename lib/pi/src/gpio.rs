...
impl Gpio<Uninitialized> {
    ...
    pub fn into_alt(self, function: Function) -> Gpio<Alt> {
        ...
        let reg = &mut self.registers.FSEL[fsel_no];
        reg.or_mask((function as u32) << fsel_shift);
        self.transition::<Alt>()
    }
...
impl Gpio<Output> {
    ...
    pub fn set(&mut self) {
        let set_no = (self.pin / 32) as usize;
        let set_shift = self.pin % 32;
        let reg = &mut self.registers.SET[set_no];
        reg.write(1 << set_shift);
    }

    /// Clears (turns off) the pin.
    pub fn clear(&mut self) {
        let clr_no = (self.pin / 32) as usize;
        let clr_shift = self.pin % 32;
        let reg = &mut self.registers.CLR[clr_no];
        reg.write(1 << clr_shift);
    }
}
...
impl Gpio<Input> {
    ...
    pub fn level(&mut self) -> bool {
        let lev_no = (self.pin / 32) as usize;
        let lev_shift = self.pin % 32;
        let reg = &mut self.registers.LEV[lev_no];
        return reg.read() & (1 << lev_shift) != 0;
    }
}
